...
from yizhi.state.migrations import SCHEMA_SQL

DEFAULT_DB_PATH = Path(".yizhi/state.sqlite")
...
def _connect(path: str | Path) -> sqlite3.Connection:
    ...
    # WAL + a busy timeout so the event store and the memory store (same db file,
    # separate connections) never raise "database is locked" under the interleaved
    # access a continuously-running agent produces.
