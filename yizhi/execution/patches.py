"""R1 — delegated patch drafting (resident-operator-plan pillar A).
...
Nothing here applies a patch. Apply is R4 — a separate, human-gated stage.
...
PROTECTED_PREFIXES = (".git/", ".yizhi/", ".env", "data/delegation/")
PROTECTED_FILES = {"will.config.toml", "yizhi.config.toml"}
...
def build_patch_instruction(instruction: str) -> str:
    ...
    return (
        ...
        "2. 所有 diff 路径以仓库根为基准(例如 a/yizhi/core/x.py),使 `git apply` 可从仓库根直接执行。\n"
        "3. 只修改与目标直接相关的文件;不得触碰 .git/ .yizhi/ data/delegation/ 或任何配置/密钥文件(will.config.toml 等)。\n"
